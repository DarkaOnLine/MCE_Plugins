...
if(isset($_SESSION['Zend_Auth']['storage']->adm_id))
{
...
<!--<script language="javascript" src="../../../jscripts/image.js"></script>-->
...
<?php
if(isset($_GET['add_link']) && $_GET['add_link']) {
    echo '
    <script type="text/javascript">
	function onOK() {
        ...
        window.opener.tinyMCE_AdvancedTheme._insertLink("javascript:void(0);","", "", "return popupimg(\'"+src+"\', "+width+","+height+");");
        top.close();
        return false;
    }
    </script>
...
<? if((isset($new_folder_auth) && $new_folder_auth || $isAdmin) && 1 && $IMConfig['safe_mode'] == false){?>
...
<!-- image properties -->
        <table class="inputTable">
                ...
                <tr>
<!-- Added by Ryan Demmer - Dec 2004. Auth and option check -->
<? if((isset($upload_auth) && $upload_auth || $isAdmin) && 1){?>
                        <td align="right"><label for="upload">Upload</label></td>
                        <td>
                                <table cellpadding="0" cellspacing="0" border="0">
                  <tr>
                    <td><input type="file" name="upload" id="upload"/></td>
                    <td>&nbsp;<input type="submit" name="submit" value="Upload" onclick="doUpload();"/></td>
                  </tr>
                </table>
                        </td>
<? } else {?>
...
<!--// image properties -->
        <div style="text-align: right;">
          <hr />
          <input type="button" class="buttons" value="Refresh" onclick="return refresh();">
          <input type="button" class="buttons" value="OK" onclick="return onOK();">
          <input type="button" class="buttons" value="Cancel" onclick="return onCancel();">
    </div>
</form>
</body>
</html>
