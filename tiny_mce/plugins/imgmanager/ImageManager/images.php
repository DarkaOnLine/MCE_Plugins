...
{
        global $relative, $imagepath, $params, $image_delete_auth, $image_edit_auth, $isAdmin;

        foreach($list as $entry => $file)
        { ?>
                <td><table width="100" cellpadding="0" cellspacing="0"><tr><td class="block">
                <a href="javascript:;" onclick="selectImage('<? echo "$imagepath".$file['relative'];?>', '<? echo $entry; ?>', <? echo $file['image'][0];?>, <? echo $file['image'][1]; ?>);"title="<? echo $entry; ?> - <? echo Files::formatSize($file['stat']['size']); ?>"><img src="<? echo $manager->getThumbnail($file['relative']); ?>" alt="<? echo $entry; ?> - <? echo Files::formatSize($file['stat']['size']); ?>"/></a>
                &nbsp;<?echo $entry ?>&nbsp;
                </td></tr><tr>
                <td class="edit">
                <? if(($image_delete_auth || $isAdmin) && 1){ ?>
...
{
        global $relative, $params, $folder_delete_auth, $isAdmin;
        ...
        { ?>
                ...
                <td class="edit">
                <? if (($folder_delete_auth || $isAdmin) && 1){ ?>
